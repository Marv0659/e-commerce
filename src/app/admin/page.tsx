...
async function getUserData() {
  ...
  return {
    userCount,
    avarageValuePerUser: userCount === 0 ? 0 : (orderData._sum.pricePaidInCents.pricePaidInCents || 0) / userCount / 100,
  };
}

async function getProductData() {
  const [activeCount, inactiveCount] = await Promise.all([db.product.count({ where: { isAvailableForPurchase: true } }), db.product.count({ where: { isAvailableForPurchase: false } })]);
...
export default async function AdminDashboard() {
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
      ...
      <DashboardCard title="Customers" subtitle={`${formatCurrency(userData.avarageValuePerUser)} Average value`} body={formatNumber(userData.userCount)} />
